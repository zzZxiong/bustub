...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (args.size() == 1 && (func_name == "lower" || func_name == "upper")) {
    return std::dynamic_pointer_cast<AbstractExpression>(std::make_shared<StringExpression>(
        std::move(args.front()), func_name == "lower" ? StringExpressionType::Lower : StringExpressionType::Upper));
  }
